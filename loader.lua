...
local cfg = {
	base = tostring(pick('base', 'BaseUrl', 'https://raw.githubusercontent.com/Floorzey/VapeTweakerForRoblox/main')):gsub('/+$', ''),
...
function ad.dt()
	...
	local ad = false
	for _, th in getreg() do
		if typeof(th) ~= "thread" then
			continue
		end
		local sr = debug.info(th, 1, "s")
		if sr and (sr:match(".Core.Anti") or sr:match(".Plugins.Anti_Cheat")) then
			ad = true
			table.insert(at, th)
		end
	end
...
function ad.bp()
	...
	local tb = {}
	if filtergc then
		local ct = filtergc("table", { Keys = { "Detected", "RLocked" } }, false)
		for _, tl in ct do
			if typeof(rawget(tl, "Detected")) ~= "function" then
				continue
			end
			table.insert(tb, tl)
		end
	else
		for _, tl in getgc(true) do
			if typeof(tl) ~= "table" then
				continue
			end
			local io = typeof(rawget(tl, "Detected")) == "function" and rawget(tl, "RLocked")
			if not io then
				continue
			end
			table.insert(tb, tl)
	...
	for _, tl in tb do
		for _, df in tl do
			if typeof(df) ~= "function" or isfunctionhooked(df) then
				continue
			end
			wax.shared.Hooks[df] = wax.shared.Hooking.HookFunction(
				df,
				function(ac, iv, no)
					coroutine.yield()
